			...
			if (!err) {
				...
				apiClient.fetch('/vehicles', {
					...
					(response) => {
						...
						this.props.history.push('/vehicles/' + response.json.id);
						notification['success']({
							message: i18n.t`Le véhicule a bien été ajouté.`,
					...
					(error) => {
						error.response.json().then(
							(body) => {
								...
								this.setState({submitting: false});
								notification['error']({
									message: i18n.t`Le véhicule n'a pas été ajouté.`,
		...
		return(
			<div className="form">
				<div className="form-content">
					<Card title={
						<div className="form-title"><Icon type="plus-circle-o" /> {'Ajouter un véhicule'}</div>
					}>
						...
						<div className="form-buttons">
							...
							<Button size="large" onClick={() => this.props.history.push('/vehicles')}>
...
export default FormComp.create()(withI18n()(AddVehicle));