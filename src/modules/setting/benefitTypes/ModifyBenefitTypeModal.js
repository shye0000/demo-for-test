...
class ModifyBenefitTypeModal extends ActionModalForm {
	constructor(props) {
		super(props, Form, <div className="modal-title">
			<EditableTransWrapper><Trans>Modifier les informations du type de prestation</Trans></EditableTransWrapper>
		</div>);
	}
	handleSubmit = (e) => {
		e.preventDefault();
		...
		this.form.validateFields((err, values) => {
			...
			apiClient.fetch(this.props.benefitType['@id'], {
				method: 'PUT',
				...
				() => {
					...
					notification['success']({
						message: i18n.t`Le type de prestation a bien été modifié.`,
				...
				(error) => {
					error.response.json().then(
						(body) => {
							...
							notification['error']({
								message: i18n.t`Le type de prestation n'a pas été modifié.`,
...
export default withI18n()(ModifyBenefitTypeModal);
