...
class ModifyVatRateModal extends ActionModalForm {
	constructor(props) {
		super(props, Form, <div className="modal-title">
			<EditableTransWrapper><Trans>Modifier les informations du taux de TVA</Trans></EditableTransWrapper>
		</div>);
	}
	handleSubmit = (e) => {
		e.preventDefault();
		...
		this.form.validateFields((err, values) => {
			...
			apiClient.fetch(this.props.vatRate['@id'], {
				method: 'PUT',
				...
				() => {
					...
					notification['success']({
						message: i18n.t`Le taux de TVA a bien été modifié.`,
				...
				(error) => {
					error.response.json().then(
						(body) => {
							...
							notification['error']({
								message: i18n.t`Le taux de TVA n'a pas été modifié.`,
...
export default withI18n()(ModifyVatRateModal);
