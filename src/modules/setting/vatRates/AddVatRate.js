...
class AddVatRate extends React.Component {
			...
			if (!err) {
				...
				apiClient.fetch('/vat_rates', {
					...
					(response) => {
						...
						this.props.history.push('/vat_rates/' + response.json.id);
						notification['success']({
							message: i18n.t`Le taux de TVA a bien été ajouté.`,
					...
					(error) => {
						error.response.json().then(
							(body) => {
								...
								this.setState({submitting: false});
								notification['error']({
									message: i18n.t`Le taux de TVA n'a pas été ajouté.`,
		...
		return(
			<div className="form">
				<div className="form-content">
					<Card title={
						<div className="form-title"><Icon type="plus-circle-o" /> <EditableTransWrapper><Trans>Ajouter un taux de TVA</Trans></EditableTransWrapper></div>
					}>
						...
						<div className="form-buttons">
							...
							<Button size="large" onClick={() => this.props.history.push('/vat_rates')}>
...
export default FormComp.create()(withI18n()(AddVatRate));
