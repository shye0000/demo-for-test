...
class ModifySkillModal extends ActionModalForm {
	constructor(props) {
		super(props, Form, <div className="modal-title">
			<EditableTransWrapper><Trans>Modifier les informations de la compétence</Trans></EditableTransWrapper>
		</div>);
	}
	handleSubmit = (e) => {
		e.preventDefault();
		...
		this.form.validateFields((err, values) => {
			...
			apiClient.fetch(this.props.skill['@id'], {
				method: 'PUT',
				...
				() => {
					...
					notification['success']({
						message: i18n.t`La compétence a bien été modifié.`,
				...
				(error) => {
					error.response.json().then(
						(body) => {
							...
							notification['error']({
								message: i18n.t`La compétence n'a pas été modifié.`,
...
export default withI18n()(ModifySkillModal);
