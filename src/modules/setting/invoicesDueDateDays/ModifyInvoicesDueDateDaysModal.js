...
class ModifyInvoicesDueDateDaysModal extends ActionModalForm {
	constructor(props) {
		super(props, Form, <div className="modal-title">
			<EditableTransWrapper><Trans>{'Modifier l\'échéance par défaut des factures'}</Trans></EditableTransWrapper>
		</div>);
	}
	handleSubmit = (e) => {
		e.preventDefault();
		...
		this.form.validateFields((err, values) => {
			...
			apiClient.fetch(this.props.invoicesDueDateDays['@id'], {
				method: 'PUT',
				...
				() => {
					...
					notification['success']({
						message: i18n.t`L'échéance par défaut des factures a bien été modifié.`,
				...
				(error) => {
					error.response.json().then(
						(body) => {
							...
							notification['error']({
								message: i18n.t`L'échéance par défaut des factures n'a pas été modifié.`,
...
export default withI18n()(ModifyInvoicesDueDateDaysModal);
