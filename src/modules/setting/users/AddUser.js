			...
			if (!err) {
				...
				apiClient.fetch('/users', {
					...
					(response) => {
						...
						this.props.history.push('/users/' + response.json.id);
						notification['success']({
							message: i18n.t`L'utilisateur a bien été ajouté.`,
					...
					(error) => {
						error.response.json().then(
							(body) => {
								...
								this.setState({submitting: false});
								notification['error']({
									message: i18n.t`L'utilisateur n'a pas été ajouté.`,
		...
		return(
			<div className="form">
				<div className="form-content">
					<Card title={
						<div className="form-title"><Icon type="plus-circle-o" /> <EditableTransWrapper><Trans>Gérer les accès d'un salarié</Trans></EditableTransWrapper></div>
					}>
						...
						<div className="form-buttons">
							...
							<Button size="large" onClick={() => this.props.history.push('/users')}>
...
export default FormComp.create()(withI18n()(AddUser));
