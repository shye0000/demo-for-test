...
class AddVehicleType extends React.Component {
			...
			if (!err) {
				...
				apiClient.fetch('/vehicle_types', {
					...
					(response) => {
						...
						this.props.history.push('/vehicle_types/' + response.json.id);
						notification['success']({
							message: i18n.t`Le type de véhicule a bien été ajouté.`,
					...
					(error) => {
						error.response.json().then(
							(body) => {
								...
								this.setState({submitting: false});
								notification['error']({
									message: i18n.t`Le type de véhicule n'a pas été ajouté.`,
		...
		return(
			<div className="form">
				<div className="form-content">
					<Card title={
						<div className="form-title"><Icon type="plus-circle-o" /> <EditableTransWrapper><Trans>Ajouter un type de véhicule</Trans></EditableTransWrapper></div>
					}>
						...
						<div className="form-buttons">
							...
							<Button size="large" onClick={() => this.props.history.push('/vehicle_types')}>
...
export default FormComp.create()(withI18n()(AddVehicleType));
