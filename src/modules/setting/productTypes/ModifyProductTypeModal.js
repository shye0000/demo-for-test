...
class ModifyProductTypeModal extends ActionModalForm {
	constructor(props) {
		super(props, Form, <div className="modal-title">
			<EditableTransWrapper><Trans>Modifier les informations du type de produit</Trans></EditableTransWrapper>
		</div>);
	}
	handleSubmit = (e) => {
		e.preventDefault();
		...
		this.form.validateFields((err, values) => {
			...
			apiClient.fetch(this.props.productType['@id'], {
				method: 'PUT',
				...
				() => {
					...
					notification['success']({
						message: i18n.t`Le type de produit a bien été modifié.`,
				...
				(error) => {
					error.response.json().then(
						(body) => {
							...
							notification['error']({
								message: i18n.t`Le type de produit n'a pas été modifié.`,
...
export default withI18n()(ModifyProductTypeModal);
